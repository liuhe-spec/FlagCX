...
flagcxResult_t flagcxGather(const void *sendbuff, void *recvbuff,
  ...
  if (is_homo_comm(comm)) {
    return cclAdaptors[flagcxCCLAdaptorDevice]->gather(
        sendbuff, recvbuff, sendcount, datatype, root, comm->homo_comm, stream);
  } else {
    char *useBootstrap = getenv("USE_BOOTSTRAP_CCL");
    if (useBootstrap) {
      // TODO: to be implemented.
    ...
    if (use_host_comm()) {
      ...
      flagcxC2cPlanner planner;
      auto hashValue =
          getC2cCommPatternHash(sendcount, comm->cluster_ids[root],
                                flagcxCommOpGather, flagcxRedNoOp, comm);
      if (!planCache.get(hashValue, planner)) {
        INFO(FLAGCX_COLL,
             "No available plan is found, create a new one with "
             "communication pattern "
             ...
             "%ld), hashValue = "
             "%ld",
             sendcount, comm->cluster_ids[root], flagcxCommOpGather,
             flagcxRedNoOp, (size_t)((uintptr_t)comm), hashValue);
        planner = flagcxC2cPlanner(sendcount, sendcount * comm->nranks,
                                   comm->cluster_ids[root], comm,
                                   flagcxCommOpGather, flagcxSum);
        FLAGCXCHECK(planner.findStrategy());
        planCache.put(hashValue, planner);
      } else {
        INFO(FLAGCX_COLL,
    ...
    if (use_host_comm()) {
      ...
      auto hashValue =
          getC2cCommPatternHash(count, comm->cluster_ids[root],
                                flagcxCommOpScatter, flagcxRedNoOp, comm);
      if (!planCache.get(hashValue, planner)) {
        INFO(FLAGCX_COLL,
             "No available plan is found, create a new one with "
             "communication pattern "
             ...
             "%ld), hashValue = "
             "%ld",
             count, comm->cluster_ids[root], flagcxCommOpScatter, flagcxRedNoOp,
             (size_t)((uintptr_t)comm), hashValue);
        planner = flagcxC2cPlanner(count * comm->nranks, count,
                                   comm->cluster_ids[root], comm,
