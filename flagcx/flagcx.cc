...
flagcxResult_t flagcxOneSideRegister(const flagcxComm_t comm, void *buff,
  ...
  if (heteroComm == NULL || heteroComm->netAdaptor == NULL ||
      heteroComm->netAdaptor->put == NULL || heteroComm->netAdaptor->regMr == NULL) {
  ...
  int recvPeer = (heteroComm->rank - 1 + heteroComm->nRanks) % heteroComm->nRanks;
  
  flagcxNetHandle_t listenHandle = {};
  void *listenComm = NULL;
  FLAGCXCHECK(heteroComm->netAdaptor->listen(heteroComm->netDev, (void *)listenHandle, &listenComm));
  

  flagcxNetHandle_t peerHandle = {};
  FLAGCXCHECK(bootstrapSend(state, recvPeer, 1001, (void *)listenHandle, sizeof(flagcxNetHandle_t)));
  FLAGCXCHECK(bootstrapRecv(state, sendPeer, 1001, (void *)peerHandle, sizeof(flagcxNetHandle_t)));
  ...
  while (sendComm == NULL || recvComm == NULL) {
    if (sendComm == NULL) {
      flagcxResult_t res = heteroComm->netAdaptor->connect(heteroComm->netDev, (void *)peerHandle, &sendComm);
      if (res != flagcxSuccess && res != flagcxInProgress) {
        INFO(FLAGCX_REG, "flagcxOneSideRegister: connect to sendPeer failed, res=%d", res);
        return res;
      }
    }
    
    if (recvComm == NULL) {
      flagcxResult_t res = heteroComm->netAdaptor->accept(listenComm, &recvComm);
      if (res != flagcxSuccess && res != flagcxInProgress) {
        INFO(FLAGCX_REG, "flagcxOneSideRegister: accept from recvPeer failed, res=%d", res);
        return res;
      }
    } 
  ...
  INFO(FLAGCX_REG, "flagcxOneSideRegister: sendComm and recvComm created, using sendComm for registration");
  
  if (heteroComm->netAdaptor->name && 
      strcmp(heteroComm->netAdaptor->name, "IB") == 0) {
    struct flagcxIbSendComm *ibSendComm = 
        (struct flagcxIbSendComm *)regComm;
    regComm = (void *)&ibSendComm->base;
  }
  
  int type = FLAGCX_PTR_HOST;
  flagcxResult_t res = heteroComm->netAdaptor->regMr(regComm, buff, size, type, &mrHandle);
  ...
  FLAGCXCHECK(bootstrapAllGather(state, (void *)info->base_vas, sizeof(uintptr_t)));
  ...
  for (int i = 0; i < nranks; i++) {
    INFO(FLAGCX_REG, "  Rank %d: base_va=0x%lx, rkey=0x%x, lkey=0x%x", 
...
flagcxResult_t flagcxCommRegister(const flagcxComm_t comm, void *buff,
  ...
  const char *enableOneSideReg = flagcxGetEnv("FLAGCX_ENABLE_ONE_SIDE_REGISTER");
