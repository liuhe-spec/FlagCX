...
flagcxResult_t buildMlx5dvSymbols(struct flagcxMlx5dvSymbols *mlx5dvSymbols) {
  ...
  ASSIGN_SYM(mlx5dvSymbols, mlx5dv_get_data_direct_sysfs_path,
             mlx5dv_internal_get_data_direct_sysfs_path);
  ASSIGN_SYM(mlx5dvSymbols, mlx5dv_reg_dmabuf_mr,
             mlx5dv_internal_reg_dmabuf_mr);
...
flagcxResult_t buildMlx5dvSymbols(struct flagcxMlx5dvSymbols *mlx5dvSymbols) {
  ...
  if (!mlx5dvhandle) {
    ...
    if (!mlx5dvhandle) {
      INFO(FLAGCX_INIT, "Failed to open libmlx5.so[.1]");
      goto teardown;
    }
  }
...
#define LOAD_SYM_VERSION(handle, symbol, funcptr, version)                     \
  ...
  LOAD_SYM(mlx5dvhandle, "mlx5dv_is_supported",
           mlx5dvSymbols->mlx5dv_internal_is_supported);
  ...
  LOAD_SYM_VERSION(mlx5dvhandle, "mlx5dv_reg_dmabuf_mr",
                   mlx5dvSymbols->mlx5dv_internal_reg_dmabuf_mr, "MLX5_1.25");
